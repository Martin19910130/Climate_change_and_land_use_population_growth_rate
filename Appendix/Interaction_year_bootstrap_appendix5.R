...
dat <- read.xlsx("C://Users/ma22buky/Documents/01_PhD/02_GCEF_demografie_ipm/Size_seedling_flower_data/transect_data_rearranged.xlsx") %>% 
  subset(species %in% c("Bro_ere", "Pla_lan", "Sca_och", "Tra_ori"))#, "Dia_car"))#, "Pla_lan", "Sca_och", "Tra_ori"))
...
dat_seeds <- read.csv("C://Users/ma22buky/Documents/01_PhD/02_GCEF_demografie_ipm/Seed_count_data/Seed_table_18_19_20_transect.csv") %>% 
  subset(year != 2022 & species %in% c("Bro_ere", "Pla_lan", "Sca_och", "Tra_ori"))#, "Dia_car"))#, "Pla_lan", "Sca_och", "Tra_ori")) 
...
aggregate(Seed_count ~ year + species + climate + management, FUN = mean, na.rm = T, data = dat_seeds) %>% subset(.,species == "Dia_car")
aggregate(fl_all ~ year + species + climate + management, FUN = sum, na.rm = T, data = dat) %>% subset(species == "Dia_car")
...
boot_lambda <- function(ii)
{
  ...
  for(i in 1:length(suber))
  {
    ...
    mean_seed <- aggregate(nr_seeds ~ year + species + climate + management, FUN = mean, na.rm = T, data = dat_mod_s)
    
    ## merge the seed data to the entire data frame by species and climate
    dat_mod <- merge(dat_mod, mean_seed[,c("species",  "year", "climate", "management", "nr_seeds")], 
                     by = c("species", "year", "climate", "management"))
    ...
    else
    {
      ...
      repr_rest <- aggregate(seeds ~ spec_year_clim + plot + subplot, FUN = sum, na.rm = T, data = dat_mod) %>% 
        merge(., sl_rest)
      ...
      repr_rest <- aggregate(new_individual ~ spec_year_clim + plot + subplot, FUN = sum, na.rm = T, data = dat_mod) %>% 
        merge(., repr_rest)
    ...
    U <- aggregate(log(size_t0) ~ spec_year_clim, FUN = max, na.rm = T, data = dat_mod) %>% rename(size = 'log(size_t0)') %>% 
      rbind(., max_t1) %>% aggregate(size ~ spec_year_clim, FUN = max, data = .)
    
    min_t1 <- tryCatch(aggregate(log(size_t1) ~ spec_year_clim, FUN = min, na.rm = T, data = dat_mod) %>% 
                         rename(size = 'log(size_t1)'), error = function(e){cat("ERROR :", conditionMessage(e), "\n")}) 
    
    L <- aggregate(log(size_t0) ~ spec_year_clim, FUN = min, na.rm = T, data = dat_mod) %>% rename(size = 'log(size_t0)') %>% 
      rbind(., min_t1) %>% aggregate(size ~ spec_year_clim, FUN = min, data = .)
...
lambda <- ggplot(mean_lambdas, aes(x = year, y = lambda, color = treatment, shape = treatment)) + 
  ...
  scale_shape_manual(values = c(18, 18, 17, 17), name = "Treatment") + 
